...
ax.plot(Mg,[tau_eff(m) for m in Mk],color=C_GEO,lw=2.4,label=r"Geometric $+$ Peierls")
...
ax.grid(True,which="major",ls=":",lw=.4,alpha=.35); ax.legend(loc="lower right",fontsize=8.5)
...
ax.set_title("When sub-cutoff PBHs evaporate (geometric channel)",fontsize=10)
...
# distortion envelope (min of mu,y where finite)
fdist=[min(a,b) for a,b in zip(fmu,fy)]
...
ax.plot(Mc,fp ,color="#b03060",lw=1.6,ls="--",label=r"this work: BBN/photodiss. (estimated)")
# Hawking present-day gamma-ray bound (schematic literature curve ~1e-8 flat in 1e16-1e17 g)
...
ax.axvspan(1e17,3e16*30,color=DM,alpha=0.08)
ax.set_xscale("log"); ax.set_yscale("log")
ax.set_xlim(1e15,1e17); ax.set_ylim(1e-9,3)
ax.set_xlabel(r"PBH mass $M$ (g)"); ax.set_ylabel(r"$f_{\rm PBH}$ (would-be DM fraction)")
ax.set_title("Constraints on sub-cutoff PBHs: geometric channel vs standard Hawking",fontsize=10)
ax.grid(True,which="major",ls=":",lw=.4,alpha=.35); ax.legend(loc="lower right",fontsize=8.0)
