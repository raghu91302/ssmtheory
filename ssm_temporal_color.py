#!/usr/bin/env python3
"""
ssm_temporal_color.py -- the temporal link does not mix color.
...
vacuum transports the color qutrit trivially.

Results:
  (1) The 24 D4 roots split 12 spatial (r4=0) + 12 temporal.  The spatial
      roots are the FCC nearest neighbors and carry a coordinate-plane
      color class; the temporal roots have spatial part along an axis and
      carry no color class at all.
  (2) A half-integer-centerd 16-cell straddles two time slices, meeting each
  ...
  (3) The unique straddling (2+2) tetrahedron on each cage bond sends that
      bond to the image of its SKEW PARTNER.  Hence
              U_t = I3 (x) sigma_x,
      which is +1 on the symmetric (color) sector and -1 on the dark
      sector.  Counting straddling tetrahedra by class gives 2*I3; the factor
      2 is the two cage bonds per class, and after normalizing the induced
      operator on the color space is I3.
      Verified for all 64 straddling cells of a 4x4x4 block.
  (4) General obstruction: color is a 3-element label, and all 48 signed
...
def sixteen_cell(center):
    return [tuple(center[i] + F(s[i],2) for i in range(4))
            for s in product((1,-1), repeat=4) if sum(s) % 4 == 0]

def temporal_color_matrix(center):
    """Induced map on the three color classes, via straddling 2+2 tetrahedra."""
    V  = sixteen_cell(center)
...
def main():
    ...
    for a,b,c in product(range(4), repeat=3):
        M = temporal_color_matrix((F(2*a+1,2), F(2*b+1,2), F(2*c+1,2), F(1,2)))
        seen[tuple(map(tuple,M))] = seen.get(tuple(map(tuple,M)), 0) + 1
    print("temporal color matrices over 64 straddling cells:")
    ...
    for p in permutations(range(3)):
        for sg in product((1,-1), repeat=3):
            ...
            acts.add(tuple(img))
    print("distinct color actions of the 48 signed permutations:", len(acts), "= |S3|")
