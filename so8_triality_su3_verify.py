...
--------------------------
INDEPENDENT NUMERICAL VERIFICATION of the theorem that su(3) is the fixed
subalgebra of a triality automorphism of so(8) twisted by an order-3 torus
element.  The theorem is proved analytically in the paper; this script checks
the conclusion by explicit construction, with no combinatorial shortcuts and no
assumed Chevalley signs.  The numbers below are evidence, not proof.  so(8) is the Lie algebra of the D4 root
lattice, whose 24 roots are the 24 nearest-neighbor bond directions.
...
Step 3 independently confirms dim Fix(tau) = 14, which is the one fact the
analytic proof imports (it fixes the Chevalley signs on the triality-fixed
roots).
...
print("\n==> su(3) = Fix( Ad(t) o triality )  on so(8):  VERIFIED numerically.")
print("    (The proof is analytic; see the paper. These are checks, not a proof.)")
